...
class TransactionController extends Controller
{
    ...
    {
        $transactions = Transaction::with('listing')->whereUserId(Auth::user()->id)->paginate();
    ...
    private function _fullyBookedChecker(Store $request)
    {
        ...
        $runningTransactionAccount = Transaction::whereListingId($listing->id)->whereNot('status', 'canceled')->where(function ($query) use ($request) {
        ...
        if ($runningTransactionAccount >= $listing->max_person) {
